...
.PHONY: prepackage
prepackage: ## Used internally to run packaging
	cargo pgx schema pg${PG_VERSION} --out sql/promscale--${EXT_VERSION}.sql
...
docker-quick-ts2-14: ## A quick way to rebuild the extension image with only SQL changes
