...
///
fn snapshot_db(container: &PostgresContainer, db: &str, username: &str, name: &str) {
...
///
fn dump_db(container: &PostgresContainer, db: &str, username: &str) {
...
///
fn restore_db(container: &PostgresContainer, db: &str, username: &str) {
    psql_file(container, db, username, Path::new("/lz/dump.sql"));
}
...
fn dump_restore_test() {
    ...
    let snapshot0 = {
        ...
        pre_dump(&container);
        snapshot_db(&container, "db", "bob", "snapshot-0.txt");
        dump_db(&container, "db", "postgres"); // todo: dump using bob user
        container.stop();
        container.rm();
        fs::read_to_string(dir.join("snapshot-0.txt")).unwrap()
    };
    ...
    let snapshot1 = {
        ...
        restore_db(&container, "db", "postgres");
        post_restore(&container);
        psql_cmd(&container, "db", "postgres", "grant prom_admin to bob;");
        snapshot_db(&container, "db", "bob", "snapshot-1.txt");
        container.stop();
        container.rm();
        fs::read_to_string(dir.join("snapshot-1.txt")).unwrap()
    };
