...
CREATE TABLE custom_log(job_id INT, time TIMESTAMP WITH TIME ZONE, value BIGINT);

CREATE OR REPLACE FUNCTION custom_func(jobid int, args jsonb) RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
    IF random() > 0.51 THEN
        PERFORM _prom_ext.push_rec(ROW(now(), 1)::_prom_ext.backend_telemetry_rec);
	RAISE EXCEPTION 'foo';
    ELSE
	PERFORM _prom_ext.push_rec(ROW(now(), 2)::_prom_ext.backend_telemetry_rec);
