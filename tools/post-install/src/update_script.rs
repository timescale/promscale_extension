...
pub(crate) fn generate_from_install(extension_file: impl BufRead, mut upgrade_file: impl Write) {
    let mut lines = extension_file
        .lines()
        .map(|line| {
            // TODO move to our code gen
            let line = line.expect("cannot read install script");
    ...
    while lines.peek().is_some() {
        ...
        if create_line.trim_start().starts_with("CREATE TYPE") {
            ...
            if create_line.trim_end().ends_with(';') {
                ...
                // ```
                create_line.push('\n');
                let alters = get_alterable_properties(&mut lines, &mut create_line);
                write_guarded_create_end(&mut upgrade_file, &type_name, &create_line, &alters);
            }
...
fn find_create_line(
    ...
    for line in lines {
        ...
        if line.trim_start().starts_with("CREATE TYPE")
            || line.trim_start().starts_with("CREATE OPERATOR")
        {
            return Some(line);
        }
...
fn extract_type_name(line: &str) -> String {
    let mut name: &str = line.split_ascii_whitespace().nth(2).expect("no type name");
    if name.ends_with(';') {
